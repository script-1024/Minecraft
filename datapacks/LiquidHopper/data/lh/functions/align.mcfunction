...
execute store result score @s lh.y run data get entity @s Pos[1] 1
...
execute store result entity @s Pos[1] double 1 run scoreboard players get @s lh.y
execute store result entity @s Pos[2] double 0.1 run scoreboard players get @s lh.z

execute if score #debug lh.settings matches 1 run tellraw @a {"nbt":"Pos","entity":"@s"}

scoreboard players set @s[tag=new] lh.try 3
execute as @e[tag=new] if score @s lh.try matches 3 run tag @s remove new