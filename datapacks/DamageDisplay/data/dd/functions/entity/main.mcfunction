...
execute if entity @s[tag=d,tag=crit] if score @s dd.timer matches 60.. run tag @s add x
...
execute if entity @s[tag=x,tag=crit] if score @s dd.timer matches 30.. run scoreboard players operation @s entity.tf.scale = @s dd.timer