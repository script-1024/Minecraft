...
execute if score @s p.exp >= @s p.xp.tmp run function std:exp/carry
execute if score @s p.exp < @s p.xp.tmp run scoreboard players operation @s p.xp += @s p.exp
execute if score @s p.exp < @s p.xp.tmp run scoreboard players set @s p.exp 0