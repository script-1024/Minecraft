...
execute as @e[tag=tracked] at @s unless entity @e[tag=fn,distance=..5] anchored eyes run summon armor_stand ^ ^0.5 ^ {Invisible:1b,Marker:1b,CustomName:' "\\u00a7f\\u00a7l最後一擊!" ',CustomNameVisible:1b,Tags:[dd,fn]}

execute as @e[tag=dd] at @s unless entity @e[tag=!dd,distance=..3] run kill @s
execute as @e[tag=dd,tag=!fn] if score @s dd.timer matches 5.. run kill @s
